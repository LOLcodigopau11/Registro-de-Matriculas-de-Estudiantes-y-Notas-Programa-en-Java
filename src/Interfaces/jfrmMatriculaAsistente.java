 ...
 * @author Paul Marthin 
 */
public class jfrmMatriculaAsistente extends javax.swing.JFrame {

    
    public jfrmMatriculaAsistente() {
        initComponents();
        setTitle("CETPRO MAGDALENA - MATRICULA - Asistente");
    ...
    private void initComponents() {
        ...
        txtCodAlumno.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                txtCodAlumnoKeyPressed(evt);
            }
        });
                        ...
                        .addGroup(MatriculaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            ...
                            .addGroup(javax.swing.GroupLayout.Alignment.LEADING, MatriculaLayout.createSequentialGroup()
                                ...
                                .addGroup(MatriculaLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(MatriculaLayout.createSequentialGroup()
                                        .addGap(6, 6, 6)
                                        .addComponent(txtTotalM, javax.swing.GroupLayout.PREFERRED_SIZE, 71, javax.swing.GroupLayout.PREFERRED_SIZE))
    ...
    private void txtTelefonoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtTelefonoKeyPressed
        ...
        // Permitir edición solo si la entrada es un dígito, retroceso o eliminar
        boolean permiteEdit = false;

        // Si el campo está vacío, solo permite '9' como primer dígito
        if (length == 0) {
            permiteEdit = c == '9';
        } else {
            // Si el primer dígito es '9', permite cualquier dígito, retrocede o elimina
            if (cel.charAt(0) == '9') {
                permiteEdit = c >= '0' && c <= '9' || evt.getKeyCode() == KeyEvent.VK_BACK_SPACE || evt.getKeyCode() == KeyEvent.VK_DELETE;
            }
        }
        // Limitar la entrada a 9 caracteres
        if (length >= 9 && c!= KeyEvent.VK_BACK_SPACE && c!= KeyEvent.VK_DELETE) {
            permiteEdit = false;
        }
        // Establece la propiedad editable del campo
        txtTelefono.setEditable(permiteEdit);

        // Consume el evento si no se permite la edición
        if (!permiteEdit) {
            evt.consume();
        }
    ...
    private void txtCodigoAlumnoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtCodigoAlumnoKeyPressed
        String code = txtCodigoAlumno.getText();
        int length = code.length();
        char c = evt.getKeyChar();

        boolean allowEdit = false;
        // Si el campo está vacío, permitir solo 'a' o 'A' como primer carácter
        if (length == 0) {
            allowEdit = c == 'a' || c == 'A';
        } else {// Si el primer carácter es 'a' o 'A', permitir cualquier dígito, retroceso o suprimir
            if (code.charAt(0) == 'a' || code.charAt(0) == 'A') {
                allowEdit = c >= '0' && c <= '9' || evt.getKeyCode() == KeyEvent.VK_BACK_SPACE || evt.getKeyCode() == KeyEvent.VK_DELETE;
            }
        }
        // Limitar la entrada a 4 dígitos después del primer carácter
        if (length > 4 && c!= KeyEvent.VK_BACK_SPACE && c!= KeyEvent.VK_DELETE) {
            allowEdit = false;
        }
        // Establecer la propiedad editable del campo
        txtCodigoAlumno.setEditable(allowEdit);
        // Consumir el evento si no se permite la edición
        if (!allowEdit) {
            evt.consume();
        }
    }//GEN-LAST:event_txtCodigoAlumnoKeyPressed

    private void txtCodAlumnoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtCodAlumnoKeyPressed
        String code = txtCodAlumno.getText();
        int length = code.length();
        char c = evt.getKeyChar();
        ...
        // Establecer la propiedad editable del campo
        txtCodAlumno.setEditable(allowEdit);
        // Consumir el evento si no se permite la edición
        if (!allowEdit) {
            evt.consume();
        }
    }//GEN-LAST:event_txtCodAlumnoKeyPressed
    ...
    public void Mostrar(String Valor){
        ...
        try (Connection conn = ConexionBD.conectarBD();
            ...
            Logger.getLogger(jfrmMatriculaAsistente.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
...
// MATRICULA
    ...
    public void mostrarM(String valor) {
        String sql = valor.isEmpty() 
                ? "SELECT m.num_matricula, m.periodo, m.mes_inicio, m.fecha, m.turno, " +
      ...
      try (Connection conn = ConexionBD.conectarBD();
          ...
          // Ajustar el ancho de las columnas
          tblMatricula.getColumnModel().getColumn(0).setPreferredWidth(10); // Numero Matricula
          tblMatricula.getColumnModel().getColumn(1).setPreferredWidth(30); // Periodo
          tblMatricula.getColumnModel().getColumn(2).setPreferredWidth(30); // Mes Inicio
          tblMatricula.getColumnModel().getColumn(3).setPreferredWidth(30); // Fecha
          tblMatricula.getColumnModel().getColumn(4).setPreferredWidth(30); // Turno
          tblMatricula.getColumnModel().getColumn(5).setPreferredWidth(100); // Alumno
          tblMatricula.getColumnModel().getColumn(6).setPreferredWidth(100); // Curso
          tblMatricula.getColumnModel().getColumn(7).setPreferredWidth(100); // Docente
          ...
          while (rs.next()) {
              Object[] fila = new Object[]{
                  ...
                  rs.getString(6), // nombre completo del alumno
                  rs.getString(7), //curso
                  rs.getString(8) // nombre completo del docente
              };
              modeloM.addRow(fila);
          }
    ...
    /*public void LlamarDatosM(){
        ...
        if (fila >= 0) {
            ...
            jcbMesInicio.setText(tblMatricula.getValueAt(fila, 2).toString());
            txtFechaMatricula.setText(tblMatricula.getValueAt(fila, 3).toString());
            txtCodAlumno.setText(tblMatricula.getValueAt(fila, 4).toString());
            jcbTurno.setText(tblMatricula.getValueAt(fila, 5).toString());
            jcbTipoCurso.setText(tblMatricula.getValueAt(fila, 6).toString());
            jcbCurso.setText(tblMatricula.getValueAt(fila, 7).toString());
            jcbDocente.setText(tblMatricula.getValueAt(fila, 8).toString());
        } else {
        }
    }*/
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(jfrmMatriculaAsistente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new jfrmMatriculaAsistente().setVisible(true);
            }
        });
    }
