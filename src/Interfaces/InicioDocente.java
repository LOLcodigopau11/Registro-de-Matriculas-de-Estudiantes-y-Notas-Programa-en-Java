...
public class InicioDocente extends javax.swing.JFrame {
    ...
    public void mostrarNotas(String Valor) {
        ...
        try {
            ...
            + "WHERE tm.cod_emp = ? AND ("
            + "ta.nombre LIKE ? OR "
            + "ta.apellido LIKE ? OR "
            + "CONCAT(ta.nombre, ' ', ta.apellido) LIKE '%" + Valor + "%' OR "
            + "tm.cod_alumno LIKE ? OR "
            + "tc.curso LIKE ? OR "
            + "tc.cod_curso LIKE '%" + Valor + "%' OR "
            + "tm.periodo LIKE '%" + Valor + "%' OR "
            + "tm.fecha LIKE '%" + Valor + "%' OR "
            + "tm.mes_inicio LIKE '%" + Valor + "%' OR "
            + "tm.turno LIKE '%" + Valor + "%' OR "
            + "CAST(tn.nota AS CHAR) LIKE ?)" // Agrega más campos según tus necesidades de búsqueda
            + "ORDER BY tn.nota ASC, tn.num_nota DESC";

            pst = conn.prepareStatement(query);
            pst.setString(1, codDocente); // Pasar el código del docente como parámetro
            pst.setString(2, "%" + Valor + "%"); // Nombre de alumno
            pst.setString(3, "%" + Valor + "%"); // Apellido de alumno
            pst.setString(4, "%" + Valor + "%"); // Código de alumno
            pst.setString(5, "%" + Valor + "%"); // Curso
            pst.setString(6, "%" + Valor + "%"); // Nota
            rs = pst.executeQuery();
    ...
    public void CalificarNota(){
        ...
        if (tblNotas.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(this, "Por favor, seleccione una fila para calificar/editar la nota.", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
